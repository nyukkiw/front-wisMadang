...
export default function CartPanel({ cart, suggestions, paymentMethod, onPaymentMethodChange, onAddSuggestion, onIncrease, onDecrease, onRemove, onPay }: CartPanelProps) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm">
      <h2 className="text-lg font-bold text-[#174a43]">Pesanan</h2>
      ...
      <div className="mt-5 border-t border-gray-100 pt-4 text-sm">
        ...
        <div className="mt-3 flex justify-between text-base font-bold text-[#174a43]">
          <span>Total</span>
          <span>Rp {total.toLocaleString("id-ID")}</span>
        </div>
      </div>
      <button type="button" disabled={cart.length === 0} onClick={onPay} className="mt-5 w-full rounded-xl bg-[#e98272] py-3 font-bold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
        Bayar Sekarang
      </button>
    </div>
  );
